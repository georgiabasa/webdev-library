...
const insertUser = (userName, password, firstName, lastName, email, phone, callback) => {
    // εισαγωγή νέου χρήστη, και επιστροφή στο callback της νέας εγγραφής
    const sql = "INSERT INTO USER(username, password, first_name, last_name, email, phoneNumber) VALUES (?,?,?,?,?,?);"
    const db = new sqlite3.Database(db_name);
    db.run(sql, [userName, password, firstName, lastName, email, phone], function (err, row){
        if (err) {
            db.close();
            callback(err, null)
        }
